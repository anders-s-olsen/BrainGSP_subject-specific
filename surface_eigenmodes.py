...
if __name__ == '__main__':
    ...
    surface_interest='native'#32k_fs_LR
    structure='midthickness'#midthickness_MSMAll
    hemispheres='L'
    hemispheres2='lh'
    num_modes=200
    save_cut=0
    is_mask=1
    mask_input_filename='BrainEigenmodes/data/template_surfaces_volumes/fsLR_32k_cortex-'+hemispheres2+'_mask.txt'
    ...
    # subjectlist2=np.loadtxt('subjectlists/subject_list_HCP_100.txt')
    # subjectlist=np.concatenate((subjectlist1,subjectlist2))
    subjectlist = np.loadtxt('subjectlists/subject_list_HCP_20.txt')
    for sub in subjectlist:
        sub = int(sub)
        # surface_input_filename='data/'+str(sub)+'/T1w/fsaverage_LR32k/'+str(sub)+'.'+hemispheres+'.'+structure+'.'+surface_interest+'.vtk'
        # # with cortex mask (remove medial wall)
        # # this is the advisable way
        # output_eval_filename='data/'+str(sub)+'/T1w/fsaverage_LR32k/'+str(sub)+'.'+hemispheres+'.'+structure+'.'+surface_interest+'_eval.txt'
        # output_emode_filename='data/'+str(sub)+'/T1w/fsaverage_LR32k/'+str(sub)+'.'+hemispheres+'.'+structure+'.'+surface_interest+'_emodes.txt'

        # # if output_eval_filename already exists, skip
        # # if os.path.exists(output_eval_filename):
        ...
        surface_input_filename='data/'+str(sub)+'/T1w/Native/'+str(sub)+'.'+hemispheres+'.'+structure+'.'+surface_interest+'.vtk'
        output_eval_filename='data/'+str(sub)+'/T1w/Native/'+str(sub)+'.'+hemispheres+'.'+structure+'.'+surface_interest+'_eval.txt'
        output_emode_filename='data/'+str(sub)+'/T1w/Native/'+str(sub)+'.'+hemispheres+'.'+structure+'.'+surface_interest+'_emodes.txt'
        calc_surface_eigenmodes_nomask(surface_input_filename, output_eval_filename, output_emode_filename, num_modes)
