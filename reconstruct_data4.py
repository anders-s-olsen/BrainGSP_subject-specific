...
def data_reconstruction_old(data,bases,recon_accuracy):
    B,M,P1 = bases.shape
    N,P2 = data.shape
    print('Starting reconstruction...')

    # only applies to SSBCAPs for subject 101107 and nr 80
...
def run_reconstruction(bases,num_modes_recon,subject,mask,parc,task_or_rest='task'):
    ...
    recon_accuracy = data_reconstruction_old(data=data,bases=bases,recon_accuracy=recon_accuracy)
...
def main(num_subs,exp_settings):
    ...
    try:
        if len(exp_settings['contrasts'])==47:
            df_acc = pd.read_pickle('reconstruction_accuracies'+num_subs+'.pkl')
        else:
            df_acc = pd.read_pickle('reconstruction_accuracies_'+exp_settings['contrasts'][0]+num_subs+'.pkl')
    except:
    ...
    for subject in subject_list:
        ...
        if len(exp_settings['contrasts'])==47:
            df_acc.to_pickle('reconstruction_accuracies'+num_subs+'.pkl')
        else:
            df_acc.to_pickle('reconstruction_accuracies_'+exp_settings['contrasts'][0]+num_subs+'.pkl')
        print('Completed '+subject+'at time '+str(datetime.datetime.now()))
